...
NSString * McKinney_Sequester_Project3$ModuleRequireFormat = @"(function(exports){"
...
@implementation McKinney_Sequester_Project3Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_mckinney_sequester_project3);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_mckinney_sequester_project3 gc];
}
...
{
	// create McKinney_Sequester_Project3 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "McKinney_Sequester_Project3" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_mckinney_sequester_project3 = [[McKinney_Sequester_Project3Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_mckinney_sequester_project3];
	
	NSString *_mckinney_sequester_project3NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _mckinney_sequester_project3NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_mckinney_sequester_project3 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:McKinney_Sequester_Project3$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.mckinney_sequester_project3.kroll" 
