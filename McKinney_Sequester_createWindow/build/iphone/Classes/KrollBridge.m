...
NSString * McKinney_Sequester_createWindow$ModuleRequireFormat = @"(function(exports){"
...
@implementation McKinney_Sequester_createWindowObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_mckinney_sequester_createwindow);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_mckinney_sequester_createwindow gc];
}
...
{
	// create McKinney_Sequester_createWindow global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "McKinney_Sequester_createWindow" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_mckinney_sequester_createwindow = [[McKinney_Sequester_createWindowObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_mckinney_sequester_createwindow];
	
	NSString *_mckinney_sequester_createwindowNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _mckinney_sequester_createwindowNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_mckinney_sequester_createwindow valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:McKinney_Sequester_createWindow$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.mckinney_sequester_createwindow.kroll" 
