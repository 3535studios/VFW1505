...
//Rare exceptions to indicate a bug in the _customfonts code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_customfonts = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_customfonts]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.customfonts.module"
