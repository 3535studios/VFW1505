...
NSString * McKinney_Sequester_Project1$ModuleRequireFormat = @"(function(exports){"
...
@implementation McKinney_Sequester_Project1Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_mckinney_sequester_project1);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_mckinney_sequester_project1 gc];
}
...
{
	// create McKinney_Sequester_Project1 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "McKinney_Sequester_Project1" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_mckinney_sequester_project1 = [[McKinney_Sequester_Project1Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_mckinney_sequester_project1];
	
	NSString *_mckinney_sequester_project1NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _mckinney_sequester_project1NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_mckinney_sequester_project1 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:McKinney_Sequester_Project1$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.mckinney_sequester_project1.kroll" 
